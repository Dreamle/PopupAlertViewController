...
#define BasePopupViewDefaultSize CGSizeMake(CGRectGetWidth(self.view.frame) * 0.75, CGRectGetHeight(self.view.frame) * 0.6)


@interface BasePopupViewController ()<UIViewControllerTransitioningDelegate,UIViewControllerAnimatedTransitioning>
{
...
- (void)viewDidLoad {
    ...
    if ([self.popupDelegate respondsToSelector:@selector(PopViewController:setupSubViewWithPopupView:)]) {
        [self.popupDelegate PopViewController:self setupSubViewWithPopupView:self.popupView];
    }
...
- (UIView *)maskView {
    if (!_maskView) {
        ...
        _maskView.backgroundColor = self.maskColor?self.maskColor:[[UIColor blackColor]colorWithAlphaComponent:0.35];
...
- (void)tapAction:(UITapGestureRecognizer *)tapGesture {
    if ([self.popupDelegate respondsToSelector:@selector(PopViewController:didTapMaskViewWithMaskView:)]) {
        [self.popupDelegate PopViewController:self didTapMaskViewWithMaskView:self.maskView];
    }
